// lab_05 — GPU-driven indirect rendering study. Condition A2 × B0.
//
...
// only WIRES it into the A2 × B0 strategy. Design + data flow: ./README.md and
...
// Data types the lab owns; they encode the A2/B0 strategy.
//
...
public:
    IndirectLab()
        : App("lab_05 - indirect rendering",
              1280,
              720,
              // Only what A2 × B0 needs (design section 7). NOT multiDrawIndirect:
              // A2 issues drawCount = 1 per call, which is allowed without it —
              // that feature is what separates A3 from A2.
              DeviceFeatures{.drawIndirectFirstInstance = true, .shaderDrawParameters = true},
...
protected:
    void onRender(const FrameContext& frame) override {
        ...
        // A2 × B0 draw loop: one indirect command per object, drawCount = 1. The
        // per-mesh bind pair that A0/A1 paid is gone; what remains is the
        // submission cost of N indirect calls, which is exactly what A3 removes.
...
private:
    ...
    // Byte-identical to lab_01/lab_03 — B0 must not drift between A conditions.
