// lab_07 — GPU-driven indirect rendering study. Condition A3 × B1.
//
//   A3 = the single buffer of A2 plus MULTI-DRAW indirect: every object's command
//        is issued by ONE vkCmdDrawIndexedIndirect with drawCount = draw count.
//        The whole frame's geometry is a single Vulkan call.
//   B1 = bindless. Not a choice here: multi-draw has no rebind point between
//        draws, so per-material descriptor sets are impossible and A3 × B0 does
//        not exist. That is why the series has 7 conditions, not 8.
//
// lab_06 vs lab_07 is THE pair that isolates multi-draw batching: same single
// buffer, same bindless materials, same shaders, same order — only the number of
// vkCmdDrawIndexedIndirect calls differs (N versus 1).
//
// The harness lives in template (lab::asset / scene / bench / render). This file
// only WIRES it into the A3 × B1 strategy. Design + data flow: ./README.md and
...
// Data types the lab owns; they encode the A3/B1 strategy.
//
// A3 keeps A2's geometry exactly: a mesh is a RANGE inside the two shared
// buffers, and these three numbers go into its indirect command. Only how the
// commands are submitted changes.
struct GpuMesh {
...
public:
    IndirectLab()
        : App("lab_07 - indirect rendering",
              1280,
              720,
              // A3 is A2 plus multiDrawIndirect — that one feature IS the
              // difference between the two conditions (design section 7).
        ...
        if (!have.multiDrawIndirect) {
            throw std::runtime_error("A3 IS multi-draw; device does not support multiDrawIndirect");
        }
        if (!have.drawIndirectFirstInstance) {
            throw std::runtime_error("A3 index path needs drawIndirectFirstInstance");
        }
        if (!have.shaderDrawParameters) {
            throw std::runtime_error("A3 index path needs shaderDrawParameters (gl_BaseInstance)");
        }

        // maxDrawIndirectCount caps one call's drawCount. It is huge on desktop
        // but not guaranteed to be, and silently drawing only part of the scene
        // would be indistinguishable from a fast condition — so read it, and
        // split into chunks if the draw list ever exceeds it.
        VkPhysicalDeviceProperties props{};
        vkGetPhysicalDeviceProperties(m_context->getPhysicalDevice(), &props);
        m_maxDrawCount = std::max(1u, props.limits.maxDrawIndirectCount);
        spdlog::info("maxDrawIndirectCount={}", m_maxDrawCount);
        ...
        createIndirect();       // A3: the static indirect buffer
        createPipeline();
        createBench();
    }
...
protected:
    void onRender(const FrameContext& frame) override {
        ...
        // A3 keeps A2's single-buffer binding: bound ONCE for the whole frame.
        // Everything a draw needs to find its mesh travels in its command.
        ...
        // A3: there is no draw loop. One call submits every object's command, so
        // the per-frame command count stops depending on N entirely. lab_06
        // records the identical commands from the identical buffer as N separate
        // calls, which is what makes the pair a clean batching measurement.
        //
        // The chunk loop is a limit guard, not the strategy: on any device where
        // maxDrawIndirectCount covers the draw list it runs exactly once.
        m_queries[slot].beginPipelineStats(frame.cmd); // inside rendering
        const uint32_t total = static_cast<uint32_t>(m_draws.size());
        for (uint32_t first = 0; first < total; first += m_maxDrawCount) {
            vkCmdDrawIndexedIndirect(handle,
                                     m_indirectBuffer.handle(),
                                     VkDeviceSize{first} * sizeof(VkDrawIndexedIndirectCommand),
                                     std::min(m_maxDrawCount, total - first),
...
private:
    ...
    void loadModels() {
        ...
        spdlog::info("A3 single buffer: {} vertices, {} indices", vertexTotal, indexTotal);
    }
    ...
    void createScene() {
        ...
        const uint32_t calls =
            (static_cast<uint32_t>(m_draws.size()) + m_maxDrawCount - 1) / m_maxDrawCount;
        spdlog::info("scene: N={} k={} meshes={} materials={} draws={} indirectCalls={}",
                     m_instances.size(),
                     MODEL_COUNT,
                     m_meshes.size(),
                     m_materials.size(),
                     ...
                     calls);
        if (calls > 1) {
            spdlog::warn("draw list exceeds maxDrawIndirectCount; A3 is issuing {} calls, "
                         "not 1 — record this with the measurement",
                         calls);
        }
    }
    ...
    // A3: one indirect command per draw, index-aligned with m_draws (and hence
    // with objects[]), byte-identical to lab_06's — the buffer is not what
    // differs between A2 and A3, only how many calls consume it.
    //
    // Culling is off in experiment 1, so it is built once and costs 0 per frame.
    // Experiment 2 is where this buffer gets rebuilt by a compute cull each frame
    // and the draw count stops being known on the CPU — that is when A3 moves to
    // vkCmdDrawIndexedIndirectCount (the drawIndirectCount feature, absent on
    // MoltenVK). Plain multi-draw is correct while the count is static.
    void createIndirect() {
    ...
    GpuBuffer m_indirectBuffer;  // A3: static, one command per draw
    uint32_t m_maxDrawCount = 1; // device limit on one call's drawCount
...
//
// NOTE — MoltenVK: Metal has no native multi-draw-indirect, so drawCount > 1 is
// emulated by encoding the commands in a loop. A3's whole premise is that one
// call replaces N, so on Apple hardware this condition may measure the emulation
// rather than the batching. Record the platform with any A2 vs A3 number.
