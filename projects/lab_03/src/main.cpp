// lab_03 — GPU-driven indirect rendering study. Condition A1 × B0.
//
...
//        Identical to lab_01, so lab_01 vs lab_03 isolates A0 vs A1.
//
// The harness lives in template (lab::asset / scene / bench / render). This file
// only WIRES it into the A1 × B0 strategy. Design + data flow: ./README.md and
...
public:
    IndirectLab()
        : App("lab_03 - indirect rendering",
              1280,
              720,
              // Only what A1 × B0 needs (design section 7): indirect draws with a
              // per-command firstInstance, and gl_BaseInstance to read it back.
              // No descriptorIndexing — B0 binds a set per material.
...
protected:
    void onRender(const FrameContext& frame) override {
        ...
        // set 0 (frame + objects) once; set 1 (material) rebinds on change (B0).
        ...
        //
        // Batches are in the shared sorted order (material, mesh), so B0 still
        // rebinds once per material and the overdraw pattern matches lab_01.
        ...
        for (const MeshBatch& batch : m_batches) {
            ...
            if (batch.material >= 0 && batch.material != boundMaterial) {
                vkCmdBindDescriptorSets(handle,
                                        VK_PIPELINE_BIND_POINT_GRAPHICS,
                                        m_pipeline.layout(),
                                        1,
                                        1,
                                        &m_materials[batch.material].set,
                                        0,
                                        nullptr); // B0
...
private:
    ...
    // Byte-identical to lab_01 — B0 must not drift between the A conditions.
    void createMaterials() {
        ...
        // sets allocated by createFrameResources() (UBO + objects SSBO).
    ...
    void createPipeline() {
        ...
        m_pipelineLayout =
            PipelineLayout::create(*m_context, {m_frameLayout.handle(), m_materialLayout.handle()});
